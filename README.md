...
## Контракт формата `Product.name`

Для единообразия во всех скриптах используйте формат **без протокола**:

- домены: `example.com`
- IP:port: `1.2.3.4:8443`

Не используйте `http://` или `https://` в `Product.name`.
Если исторические данные уже содержат URL с протоколом, вспомогательные скрипты нормализуют их к виду без протокола во время обработки.
